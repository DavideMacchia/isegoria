...
- **REPUTATION-004 (update).** The asymmetric update penalizes variance: an honest
  ...
  long-window mean (`reputation::EvaluatorHistory`), which tracks the true mean, and a
  one-sided CUSUM (`k = 0.03`, `h = 1.5`) returns a changed reviewer to probation
  (`AT-REP-07`: about one false alarm per 10,000 honest items; a 20% flipper caught in
  every seeded stream, median 50 items); `asymmetric_ema` removed.
...
| Reputation (C) | Beta-shrinkage `C_a`; leave-one-out difference score `S_u`, odds weights `exp(γ·S_u·k/(k+k_0))`, long-window mean + CUSUM, `w_max = 3·median` (D33, D34, D36) | `C_a` exact; `S_u`, the odds weights, `EvaluatorHistory` with the CUSUM, the cap, probation 30 (T50, T51) | Matches the sim's scores to 1e-6; docs examples; AT-REP-02/03/04/05/07 | the sim's evaluators block (scores and weights) | Log score; live outcomes with exploration (D35, T52) | — | Formula-level, the properness enumeration, the CUSUM rates on the reference regime |
...
#### REPUTATION-004 — Reputation dynamics: long-window mean and change detector (D34)
- **Claim.** The weight reads the symmetric long-window mean of the reviewer's per-item scores (`EvaluatorHistory::score`); a one-sided CUSUM against the reviewer's own mean of the previous items, `s ← max(0, s + (S̄_u − S_uj) − k)` with `k = 0.03`, raises an alarm at `s > h = 1.5` (both provisional): the history restarts and the reviewer returns to probation (`ReviewerStanding::from_history`; a founder loses its seed weight at the first alarm). `honeypot::record_golden_scores` feeds the golden items' scores in.
- **Why.** The asymmetric update penalized variance, not error: on the paper's reference regime (nine forecasters sharing a crowd error, the reviewer cautious and better than the crowd) it held the honest reviewer at −0.10 against a true mean of +0.002 while a crowd copier stayed at 0 (paper §5.5; `change_detector.rs` reproduces the update inline as the record of the defect).
- **Evidence.** `change_detector.rs` (AT-REP-07): a seeded honest stream of 10,000 items raises at most one alarm (about one per 10,000 across seeds, with the running mean as reference — the paper's 0.07 per 1,000 used the true mean), and the long-window mean lands within 0.01 of the true mean; a reviewer who starts flipping 20% of its forecasts after 300 items is caught in all 30 seeded streams — median 50 items, 25 of 30 within 100, worst 224. `level_c.rs`, `properties.rs`: the history by hand and by property (the first item only seeds the mean, the statistic never negative, an alarm restarts the record and is counted, the mean within the recorded scores' range). `adversarial.rs::a_long_con_is_unprofitable`: 300 honest items then a betrayal costing ≈ 0.15 per item, caught within 100 items, the record wiped, the cap on an outlier. `orchestrator_driver.rs`: a caught reviewer, founder or not, weighs 0.
- **Evidence status.** TESTED (the mechanism and its rates on the reference regime). The game-theoretic claim (AT-REP-01) stays a HYPOTHESIS; `k`, `h` are provisional (T25).
...
`C_a` as in REPUTATION-001. Evaluator score (D33, T50): `S_uj = (p̄_{−u,j} − o_j)² − (p_uj − o_j)²`, `p̄_{−u,j} = Σ_{v≠u} w_v p_vj / Σ_{v≠u} w_v` (the plain mean of the others when they carry no weight; nothing scored in a panel of one); `S_u = mean_j S_uj`; `w_u = exp(γ · S_u · k_u/(k_u + k_0))`, `γ = 35`, `k_0 = 100`, both provisional; cap `min(w_u, 3·median{w > 0})` across the epoch (REPUTATION-005). History (D34, T51): `S_u` = the mean of the per-item scores since the last restart; CUSUM `s ← max(0, s + (S̄_u − S_uj) − k)` with `S̄_u` the mean of the previous items, `k = 0.03`; `s > h = 1.5` → alarm, the history restarts, probation. Dasgupta–Ghosh: binary agreement minus baseline, per pair; no aggregation over pairs/items specified. Bayesian Truth Serum: not implemented.
**Required.** The definition of `q_j`; a bound on the crowd-sabotage exposure of the leave-one-out baseline (a coalition degrading its own forecasts, paper §5.4); the values of `γ`, `k_0`, `k`, `h` from T24/T25; the game-theoretic analysis of the long con (AT-REP-01).
...
| `Established` | epoch close | — | `Established` | `S_u` = the long-window mean of the per-item scores since the last restart (✓ T50/T51: `EvaluatorHistory`, fed the golden items by `honeypot::record_golden_scores`; live outcomes and exploration are T52); `w = min(3·median{w > 0}, exp(γ·S_u·k_u/(k_u+k_0)))` — the cap binds (✓ T50, REPUTATION-005) |
| `Established` | CUSUM alarm: the per-item scores fall below the reviewer's own mean for long enough (`s > h`, `k = 0.03`, `h = 1.5`) | — | `Probation{0}` | the history restarts and the alarm is counted; a founder's seed weight is lost (✓ T51, D34) |
...
| Long-con (accumulate then spend) | MITIGATED at the mechanism level (T5, T50, T51): the weight is consumed by the fit, the cap binds on the odds scale, and the CUSUM returns a reviewer to probation at the first sustained fall — the hoarded record is wiped (a 20% flipper is caught after 50 items in the median); the game-theoretic best-response analysis (AT-REP-01) is still open | `adversarial.rs`, `change_detector.rs` |
...
| AT-REP-01 | long-con, game-theoretic | agent maximizing Σ_t influence·(betrayal payoff) under the long-window mean, the CUSUM (`k`, `h`) and the cap, with influence actually consumed by bridging | best response is honesty | REPUTATION-004 |
...
| AT-REP-07 ✓ | change detection (D34) | the paper's regime — nine forecasters sharing a crowd error, the reviewer cautious and better than the crowd — seeded: an honest stream of 10,000 scored items; 30 streams in which the reviewer starts flipping 20% of its forecasts after 300 items (`change_detector.rs`, T51) | at most one alarm on the honest stream (≈ 1 per 10,000 across seeds, running-mean reference); the flipper caught in every stream — median 50 items, 25 of 30 within 100, worst 224; the long-window mean within 0.01 of the true mean where the asymmetric update sat at −0.10 | REPUTATION-004 |
...
│   ├── change_detector.rs      (AT-REP-07; the D34 history and CUSUM, T51)
├── simulations/
...
| REPUTATION-004 | the dynamics deter the long con | `reputation::EvaluatorHistory`; `change_detector.rs` (AT-REP-07), `adversarial.rs`, `orchestrator_driver.rs` | TESTED (T51, D34) — the long-window mean tracks the true mean; the CUSUM catches a 20% flipper (median 50 items) at ≈ 1 false alarm per 10,000; an alarm returns the reviewer to probation | game analysis; `k`, `h` calibrated | AT-REP-01, T25 |
...
| Reputation | §6.7 | `crates/scoring/src/reputation.rs`: `AuthorPrior`, `author_score`, `proposal_rate`, `loo_baseline`, `difference_scores`, `mean_score`, `odds_weight`, `EvaluatorHistory` (`CUSUM_K`, `CUSUM_H`), `weight_cap`, `capped_weight`, `cap_weights`, `dasgupta_ghosh` | `level_c.rs`, `proper_score.rs`, `change_detector.rs`, `scoring/tests/adversarial.rs` | evaluators block |
