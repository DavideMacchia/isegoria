...
1. **Mathematics** (`02`, `01` D32–D41) — after the severe defect of the protocol
   boundary that is left (respondents without an identity gate; the replayed deposit,
   T64, is fixed), correct the scoring mechanism and the decisions built on it (band,
   appeal), make the engine robust to malformed input, and characterize every
   threshold. It is the genuinely new piece, and everything else consumes its numbers.
