...
- **Done:** the audit's six concrete defects (`docs/08` §0-bis); the decisions D17–D41;
  ...
  precondition of the latent re-check (D37, T53), the proper evaluator score with
  odds-scale weights and the 30-outcome probation (D33, D36, T50) and the long-window
  mean with the CUSUM change detector (D34, T51). Details and evidence:
  [Completed work](#completed-work).
- **Open:** the design revisions D35, D38–D41 and the model half of D37 (done: D32, T49;
  D33 and D36, T50; D34, T51; D37's precondition, T53); the defects found by the third
  review (2026-09-24), all but T64, T65, T62, T59 and T61; the network runtime
...
epoch. The decisions D32–D41 correct them (done: D32, T49; D33 and D36, T50; D34, T51; D37's precondition, T53). Several change the
...
| T54 | **Latent DIF with θ inside the likelihood.** Mixture IRT that integrates θ (quadrature) and includes the anchors with class-invariant parameters; re-derive the verdict threshold on it; retire the proxy-θ model. Regenerate the mixture fixtures with at least 40 anchors and the anchor matrix exported (today's carry θ from 30 anchors, below T53's floor) | D37; DIF-006/008/010 | `AT-DIF-01` holds (item-level false-positive rate ≤ 5% on null batches) for anchor KR-20 from 0.80 to 0.95 at N = 3,000–12,000, and `AT-DIF-12` (campaigns with 2, 4 and 6 of 8 items biased are flagged correctly) passes; runtime per batch recorded | L |
| T55 | **Contested-facts pool.** An item with DIF whose key a primary source establishes (evidence procedure of `docs/02` §B.5) becomes a *contested fact*: separate pool, drawn only in balanced sets so that differential test functioning stays within a tolerance; define the DTF statistic and the procedure in `docs/02`/`docs/05` first — a docs task inside this phase; the implementation is the last item of the phase (Milestone 1b), since it adds a pool and a policy rather than correcting a number | D38; paper §4.7 | `AT-PRO-08`: every assembled test has DTF within tolerance; a DIF item without a verified source is rejected as before | L |
...
6. T53 (S) — done (2026-09-25): the anchor-reliability gate and the differential-gap
   diagnostic.
7. T50 (M) and T51 (M) — done (2026-09-25): the proper evaluator score, odds weights
   and probation 30; the long-window mean with the CUSUM change detector. The
   mutation-testing re-run scheduled after this step (`docs/11`) is pending.
8. T60 (L) — the supplementary review with real extra reviewers.
9. T56 (M), then T57 (M) — coordination on residuals, then panel diversification.
10. T52 (L) — live outcomes with exploration.
11. T54 (L) — latent DIF with θ inside the likelihood.
12. T39 (S if descoped), T45 (M).
...
- **Phase 1.** T64 and T65, the two severe defects, are done, and so are T62 and T49
  ...
  followed the D27 decision and is done. T50 and T51 are done; T52 adds scored outcomes
  (D35) to the histories they keep. T53 is done; T54 builds
  on its gate and regenerates the mixture fixtures with reliable anchors. T55
...
| T50 | **Proper evaluator score, odds weights, short probation. Done (2026-09-25):** `scoring::reputation::loo_baseline` (the other panelists' weight-adjusted mean forecast — D23's crowd minus the reviewer; the plain mean of the others when they carry no weight; empty for a panel of one), `difference_scores` (`S_uj = (p̄_{−u,j} − o_j)² − (p_uj − o_j)²` per panelist and item), `mean_score`, `odds_weight` (`exp(γ · S · k/(k + k_0))`, `GAMMA = 35`, `K_SHRINK = 100`), `cap_weights` (`3 × median` of the positive weights, applied to all); `honeypot::reviewer_skills` returns the mean difference score per panelist; `probation::N_PROBATION = 30` and `review_weight(status, score, scored)` returns the uncapped odds weight; `orchestrator::ReviewerStanding { is_founder, scored, score }`, `bridging_weights(prev)` computes the odds weights and caps them across the epoch, `weighted_ratings(r, mask, prev)`; `brier_skill_score`, `base_rate_baseline`, `crowd_baseline`, `evaluator_score` removed. `sim/bridging_irt_dif.py` (evaluators block) and `sim/export_fixtures.py` compute the new score; `levelc_scores.csv` (profile, leave-one-out score, weight — expert +0.21 / 1.94, peer-follower −0.36 / 0.31, partisan −0.26 / 0.44) replaces `levelc_bss.csv`, every other fixture reproduced byte for byte under the pinned environment. *Probe on the previous code:* with one golden item and the crowd at 0.65, a reviewer who believed 0.30 maximized the BSS-based skill by reporting 0.57 (+0.016 against −0.141 for the truth); no `E_u ∈ (0,1)` was ever capped; probation was 200. *Evidence:* `proper_score.rs` — AT-REP-05 by exact enumeration (`m ≤ 4`, 50 instances × 20 alternative reports: the loss of any other report equals its mean squared distance from the belief) and the retired rule's optimum `logit q + 2 logit b` reproduced; `level_c.rs` — the oracle to 1e-6, AT-REP-02 (a copier scores exactly 0 on every item for every outcome, weight 1), AT-REP-03 (finite with a perfect crowd), the D33 examples (×2.01; ×2.4 vs ×1.13), AT-REP-04 (an outlier at 4.06 held at 3, probationers' zeros not counted); `hand_computed.rs`, `properties.rs` (the leave-one-out crowd within the others' range and unmoved by the reviewer's own forecast; scores in [−1, 1]); `lifecycle.rs`, `orchestrator_driver.rs` (probation 30, weights and the cap at the protocol boundary). *Left to T51/T52:* the asymmetric EMA is still in the crate (not wired), scored outcomes are the golden items only; `γ`, `k_0` provisional (T24/T25); the mutation-testing re-run after step 7 (`docs/11`) is pending | D33, D36; paper Props 12, 14, 15, §7.2; REPUTATION-005/008, G-12 | `AT-REP-05`, `AT-REP-02` and `AT-REP-04` pass; probation ends at 30 scored outcomes | M |
| T51 | **Change detector instead of the asymmetric update. Done (2026-09-25):** `scoring::reputation::EvaluatorHistory { sum, scored, cusum, alarms }` — `record(x, k, h)` adds a per-item score to the long-window mean (the `S_u` of D33) and updates the one-sided CUSUM against the mean of the previous items, `s ← max(0, s + (S̄_u − x) − k)`; at `s > h` the alarm is counted and the history restarts (`score()`, `scored()`, `cusum()`, `alarms()`); `CUSUM_K = 0.03`, `CUSUM_H = 1.5` (provisional); `asymmetric_ema` removed. `honeypot::record_golden_scores(histories, predictions, weights, outcomes) -> Vec<bool>` feeds the golden items' per-item scores in and reports the alarms; `orchestrator::ReviewerStanding::from_history(is_founder, &history)` — a caught reviewer has `scored = 0` (probation), and a founder loses its seed weight at the first alarm. *Probe on the previous code* (the paper's regime, `revisions_evaluator.py::panel_bias`, ported with `ChaCha8`): the asymmetric update (0.02 / 0.2) held the cautious honest reviewer at −0.10 against a true mean of +0.002, below a crowd copier's 0. *Evidence:* `change_detector.rs` (AT-REP-07) — a seeded honest stream of 10,000 items: at most one alarm (≈ 1 per 10,000 across seeds with the running mean as reference; the paper's 0.07 per 1,000 used the true mean), the long-window mean within 0.01 of the truth; a reviewer flipping 20% of its forecasts after 300 items caught in 30 of 30 seeded streams, median 50 items, 25 of 30 within 100, worst 224; `level_c.rs` (the history by hand), `properties.rs` (the mean within the scores' range, the statistic never negative, alarms restart and are counted), `adversarial.rs::a_long_con_is_unprofitable` (300 honest items then a betrayal costing ≈ 0.15 per item: caught within 100, the record wiped; the cap on an outlier), `orchestrator_driver.rs` (a caught reviewer, founder or not, weighs 0 through `bridging_weights`). *Deviation from the row's target:* "caught within 100 scored items" holds for 25 of 30 seeded streams (median 50), not all; recorded in `docs/08` AT-REP-07. *Left:* `k`, `h` calibrated (T25); the game-theoretic AT-REP-01; the mutation-testing re-run after step 7 (`docs/11`) | D34; paper §5.5, §7.2, Table 12; REPUTATION-004 | `AT-REP-07`: at most one alarm on the honest stream; the flipper caught within 100 scored items (the game-theoretic `AT-REP-01` stays open) | M |
| T53 | **Anchor-reliability gate for latent DIF. Done (2026-09-25):** `scoring::irt::kr20` — the Kuder–Richardson 20 of the anchor total on the batch's respondents (`K/(K−1)·(1 − Σ p_j(1−p_j)/Var(T))`, population variance, as `paper/scripts/revisions_dif.py`; 0 when undefined, never NaN) — and `KR20_MIN = 0.90`; `protocol::pilot::admit_anchors(anchors) -> Result<θ, PilotError>` refuses `PilotError::UnreliableAnchors { kr20, anchors }` below the floor and otherwise returns `theta_from_anchors`; `revalidation::revalidate_batch_latent(respondents, anchors, responses, seed)` takes the anchor responses instead of a θ, checks the item floor, the respondent floor and the shapes first (D37: "like the respondent and item floors"), then the anchors, and derives θ itself — no caller can hand it an unchecked proxy; `MixtureDif::differential_gap` — per counted class pair the signed gap `b_jg − b_jh` less its median over the batch's items, the largest over pairs (the paper's `2\|δ_j − median δ\|` with two classes) — is reported and never read by `latent_flags`. *Probe on the previous code* (the paper's null-batch design, N = 6,000, seeds 1300–1301): 10 anchors, KR-20 0.70–0.73 → a two-class mixture flagging 7–8 clean items of 8; 20 anchors, 0.83 → 2–3 flagged; 60 anchors, 0.93 → one class, nothing flagged — the re-check ran regardless. *Evidence:* `protocol/tests/anchor_reliability.rs` — AT-DIF-11: 10 and 20 anchors refused with the engine's KR-20 in the error, 60 admitted with no flag; the gate is the last precondition (a batch of one, a short sample, a missing or ragged anchor row are reported first); `documents_limitation`: the ungated detector flags clean items from 10 anchors; AT-DIF-12 (diagnostic half): with 60 anchors the verdict flags exactly the shifted items at 2, 4 and 6 of 8 while the differential gap agrees at 2 (1.5–2.0 vs ≤ 0.2), cannot separate at 4 (every item 0.86–0.98) and inverts at 6 (≤ 0.20 on the shifted, 1.90 on the clean). `scoring/tests/anchor_reliability.rs`: KR-20 hand-computed (0.75, 1, −1), the degenerate cases, the Level-B fixture's 30 anchors at 0.816 — below the floor; `scoring/tests/latent_classes.rs`: the differential gap agrees with the raw gap on the 3-of-8 fixture; unit tests of the gap arithmetic on two- and three-class parameter vectors; `golden_bits.txt` gains the 16 `mixture_*.differential_gap` rows, no existing value moved. *Left to T54:* the mixture fixtures carry θ from 30 anchors and no anchor matrix, so the oracle tests reach the detector through `revalidate_pool_latent`; the floor is provisional (T24/T25); the artefact is refused, not removed | D37; paper Prop 10, Tables 6, 14, 15; DIF-010 | `AT-DIF-11`: the paper's null batches with 20 anchors are refused, those with 60 are accepted and raise no flag | S |
