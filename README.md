...
| [`identity`](crates/identity) | Anonymous enrollment: source adapters, threshold-issued credential, role nullifiers | Scaffold + real mechanisms (single-server + threshold OPRF label, single + threshold BBS+ blind credential, hash nullifiers) |
...
t-of-n (a DKG + base-OT + MPC signing protocol, so no `t-1` members can issue); and
anchoring on the real **OpenTimestamps** proof format (via `opentimestamps`). What
remains is a real distributed key-generation ceremony and transport for the two
committees, the live network side of anchoring (a calendar server and a Bitcoin block
source), plus Semaphore ZK nullifiers, peer-to-peer transport (libp2p) and convergent
state (CRDT).
...
| Identity, network, protocol | Working scaffolds; deterministic mechanisms + single-server & threshold OPRF label + single & threshold BBS+ credential + OpenTimestamps anchoring proofs real, remaining heavy crypto/transport behind traits |
| Real crypto/transport integration (Semaphore, committee DKG/transport, libp2p, live OpenTimestamps calendar/Bitcoin) | Future work |
