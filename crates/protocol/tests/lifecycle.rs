...
fn honeypot_injects_about_the_target_rate_and_catches_random_voters() {
    ...
    // Each is scored against the other's forecast (its crowd, D33): the expert beats a
    // coin-flipper by 0.25 − 0.05² per item; the coin-flipper loses the same amount.
    let skills = reviewer_skills(&[expert, random], &[1.0, 1.0], &outcomes);
    assert!((skills[0] - 0.2475).abs() < 1e-9, "expert: {}", skills[0]);
    assert!(
        (skills[1] + 0.2475).abs() < 1e-9,
        "random voting: {}",
        skills[1]
    );
}
...
fn probation_gates_weight_until_a_track_record_exists() {
    // Probation ends at 30 scored outcomes (D36, T50), not 200.
    assert_eq!(N_PROBATION, 30);

    // A new node is on probation: measured, but weight 0 (docs/03 P2), however good.
    assert_eq!(status(false, 0), Status::Probation);
    assert_eq!(status(false, N_PROBATION - 1), Status::Probation);
    assert_eq!(effective_review_weight(false, 20, 0.05), 0.0);

    // A founder seeds the bootstrap at uniform weight 1 (docs/05 cold start).
    assert_eq!(status(true, 0), Status::Founder);
    assert_eq!(effective_review_weight(true, 0, 0.05), 1.0);

    // Past the threshold, anyone carries the odds weight of its score with shrinkage
    // (D33): exactly 1 at the crowd's level; 0.02 better over 400 outcomes is
    // exp(35 · 0.02 · 400/500) ≈ 1.75; worse than the crowd, below 1.
    assert_eq!(status(false, N_PROBATION), Status::Established);
    assert_eq!(status(true, N_PROBATION), Status::Established);
    assert_eq!(effective_review_weight(false, N_PROBATION, 0.0), 1.0);
    let better = effective_review_weight(true, 400, 0.02);
    assert!((better - 0.56f64.exp()).abs() < 1e-9, "{better}");
    assert!(effective_review_weight(false, 400, -0.02) < 1.0);
}
...
fn founder_set_tracks_declared_members() {
    ...
    assert_eq!(
        effective_review_weight(founders.contains(&carol), 0, 0.05),
        0.0
    );
    assert_eq!(
        effective_review_weight(founders.contains(&alice), 0, 0.05),
        1.0
    );
}
