...
/// A stand-in action context; the properties under test do not depend on its value.
const CTX: &[u8] = b"test-action";
...
fn a_valid_nullifier_proof_verifies() {
    ...
    assert!(verify(&proof, &issuer.public(), CTX));
}
...
fn same_person_and_role_give_the_same_nullifier() {
    ...
    let p1 = prove(&cred, &issuer.public(), Role::Judge, CTX);
    let p2 = prove(&cred, &issuer.public(), Role::Judge, CTX);
    assert!(verify(&p1, &issuer.public(), CTX));
    assert!(verify(&p2, &issuer.public(), CTX));
...
fn different_roles_give_different_nullifiers() {
    let (issuer, cred) = issued();
    let judge = prove(&cred, &issuer.public(), Role::Judge, CTX);
    let propose = prove(&cred, &issuer.public(), Role::Propose, CTX);
...
fn different_people_get_different_nullifiers() {
    ...
    let a = prove(&mk([9u8; 32]), &issuer.public(), Role::Judge, CTX);
    let b = prove(&mk([10u8; 32]), &issuer.public(), Role::Judge, CTX);
...
fn a_proof_does_not_verify_under_a_different_issuer() {
    ...
    let proof = prove(&cred, &issuer.public(), Role::Judge, CTX);
    assert!(verify(&proof, &issuer.public(), CTX));
    assert!(!verify(&proof, &other.public(), CTX));
}
