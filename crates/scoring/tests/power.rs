...
fn detection_rate(nt: usize, seeds: u64) -> f64 {
    ...
    for s in 0..seeds {
        ...
        let biased = res.delta[..n_biased].iter().sum::<f64>() / n_biased as f64;
        let clean = res.delta[n_biased..].iter().sum::<f64>() / (k - n_biased) as f64;
        if biased > 0.5 && clean < 0.5 && res.bic > 0.0 {
            hits += 1;
        }
    }
