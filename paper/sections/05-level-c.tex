...
design expects. The repository audit notes the same point. Rescaling does not help:
relative weights $E_u/\operatorname{median}(E)$ stay below $1/\operatorname{median}(E)\le3$ under
the same condition. The cap needs an unbounded scale, such as the odds
$w_u=E_u/(1-E_u)=e^{\gamma\,\mathrm{BSS}_u}$, or $e^{\gamma S_u}$ with the score of
\cref{prop:loo} \openq.
