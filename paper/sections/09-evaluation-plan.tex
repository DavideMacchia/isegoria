...
The findings above, and the revisions that answer them, are single-regime results. We commit in
advance to the following study before any threshold is declared final, including the new
provisional values: the side-balanced threshold $0.80$, $\gamma\approx35$, $k_0\approx100$, the
CUSUM parameters, the exploration rate and the KR-20 floor. The study extends roadmap items T24
and T25 of the repository.
...
penalty ratio (5--500); the score read by the gate (intercept, and the side-balanced score of
\cref{prop:side} under 90/10 splits, overlapping camps and a continuum of positions). We will also vary batch composition:
...
We will also compare the asymmetric update with the CUSUM of D34, and scoring on golden items
alone with scoring on explored live outcomes (D35).
\emph{Metrics:} drift of reports toward the crowd; rank correlation between $E_u$ and true
skill; time to detect a long-con reviewer; false alarms per scored item.
