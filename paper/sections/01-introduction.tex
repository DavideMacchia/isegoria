...
\begin{enumerate}
  ...
  \item \textbf{Adopted revisions} (\cref{sec:revisions}, new in version 0.2). Each finding comes
    with the correction adopted in the repository (decisions D32--D41) and the experiments that
    support it. Among them are a side-balanced bridge score, invariant under every gauge and exactly
    camp-balanced in the two-camp model (\cref{prop:side}), and randomized exploration that keeps
    the evaluator score proper when outcomes are selected by the gate (\cref{prop:ipw}).
